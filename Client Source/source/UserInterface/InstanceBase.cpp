...
{
	const auto pHour = GetPlayTime() / 60;
	auto rutbe = pHour / 2;

	if (rutbe >= 55) { rutbe = 55; }
		
	char rutbeYol[MAX_PATH];
	sprintf(rutbeYol, "rutbeler/rutbe_%d.png", rutbe);
	return rutbeYol;
}
#endif
...
{
	float fRatio = 1 + CPythonSystem::Instance().GetStoneScale();
	if (fRatio <= 1.0f)//Min Scale
	{
		m_GraphicThingInstance.SetScale(1.0f, 1.0f, 1.0f, true);
	}
	else if (fRatio >= 2.0f)//Max Scale
	{
	...
	D3DXVECTOR3 TempVec = m_GraphicThingInstance.GetScale();
	if (!(fRatio == TempVec.x && fRatio == TempVec.y && fRatio == TempVec.z))
	{ 
		m_GraphicThingInstance.RefreshActorInstance();
	}
