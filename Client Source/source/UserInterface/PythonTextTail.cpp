...
void CPythonTextTail::ArrangeTextTail()
{
	...
	for (itor = m_CharacterTextTailList.begin(); itor != m_CharacterTextTailList.end(); ++itor)
	{
		...
		CGraphicImageInstance * pPlayTime = pTextTail->pPlayTimeInstance;
#endif
		...
		CGraphicImageInstance * pGameMaster = pTextTail->pGameMasterIconInstance;
#ifdef __GM_ICON__
		if (pGameMaster)
		{
			CGraphicTextInstance* pLevel = pTextTail->pLevelTextInstance;
			...
			if (pLevel)
			{
				...
				{
					pTitle->GetTextSize(&iTitleWidth, &iTitleHeight);
					pGameMaster->SetPosition(pTextTail->x + (iNameWidth / 2) - 4.0f + iTitleWidth + iLevelWidth - 15.0f - pGameMaster->GetWidth() / 2 + 8.5f, pTextTail->y - 10);
				}
				if (!pTitle)
					pGameMaster->SetPosition(pTextTail->x + (iNameWidth / 2) - 4.0f + iLevelWidth - 15.0f - pGameMaster->GetWidth() / 2 + 8.5f, pTextTail->y - 10);
			}
		}
#endif
		...
		if (pPlayTime)
		{
			...
			if (pLevel)
			{
				...
				if (pTitle)
				{
					...
					pTitle->GetTextSize(&iTitleWidth, &iTitleHeight);
					pPlayTime->SetPosition(pTextTail->x - (iNameWidth / 2) - 4.0f + newX - iTitleWidth - iLevelWidth - 15.0f - pPlayTime->GetWidth() / 2 + 8.5f, pTextTail->y - 15);
				}
				if (!pTitle)
					pPlayTime->SetPosition(pTextTail->x - (iNameWidth / 2) - 4.0f - iLevelWidth - 15.0f - pPlayTime->GetWidth() / 2 + 8.5f, pTextTail->y - 15);
			}
		}
#endif
...
#endif
#ifdef __GM_ICON__
		if (pTextTail->pGameMasterIconInstance)
		{
			pTextTail->pGameMasterIconInstance->Render();
		}
#endif
...
void CPythonTextTail::RegisterCharacterTextTail(DWORD dwGuildID, DWORD dwVirtualID, const D3DXCOLOR & c_rColor, float fAddHeight)
{
...
#endif
#ifdef __GM_ICON__
	pTextTail->pGameMasterIconInstance=NULL;
#endif
	if (0 != dwGuildID)
	{
	...
	{
			pTextTail->pGameMasterIconInstance = CGraphicImageInstance::New();
			char szPath[256];
			sprintf(szPath, "d:/ymir work/gm.tga");
			pTextTail->pGameMasterIconInstance->SetImagePointer((CGraphicImage*)CResourceManager::Instance().GetResourcePointer(szPath));
	}
#endif
	

#ifdef ___SALTANAT_SEVIYEYE_GORE_IKON___
	if (pCharacterInstance->IsPC() && pCharacterInstance->GetLevel() > 100 &&  pCharacterInstance->GetLevel() <= 102)
	{
	...
	else if (pCharacterInstance->IsPC() && pCharacterInstance->GetLevel() > 102 &&  pCharacterInstance->GetLevel() <= 104)
	{
			pTextTail->pSaltanatFlagInstance = CGraphicImageInstance::New();
			char szPath[256];
	...
	else if (pCharacterInstance->IsPC() && pCharacterInstance->GetLevel() > 104 &&  pCharacterInstance->GetLevel() <= 106)
	{
			pTextTail->pSaltanatFlagInstance = CGraphicImageInstance::New();
			char szPath[256];
	...
	else if (pCharacterInstance->IsPC() && pCharacterInstance->GetLevel() > 106 &&  pCharacterInstance->GetLevel() <= 108)
	{
			pTextTail->pSaltanatFlagInstance = CGraphicImageInstance::New();
			char szPath[256];
	...
	else if (pCharacterInstance->IsPC() && pCharacterInstance->GetLevel() > 108 &&  pCharacterInstance->GetLevel() <= 110)
	{
			pTextTail->pSaltanatFlagInstance = CGraphicImageInstance::New();
			char szPath[256];
	...
	else if (pCharacterInstance->IsPC() && pCharacterInstance->GetLevel() > 110 &&  pCharacterInstance->GetLevel() <= 112)
	{
			pTextTail->pSaltanatFlagInstance = CGraphicImageInstance::New();
			char szPath[256];
	...
	else if (pCharacterInstance->IsPC() && pCharacterInstance->GetLevel() > 112 &&  pCharacterInstance->GetLevel() <= 114)
	{
			pTextTail->pSaltanatFlagInstance = CGraphicImageInstance::New();
			char szPath[256];
	...
	else if (pCharacterInstance->IsPC() && pCharacterInstance->GetLevel() > 114 &&  pCharacterInstance->GetLevel() <= 116)
	{
			pTextTail->pSaltanatFlagInstance = CGraphicImageInstance::New();
			char szPath[256];
	...
	else if (pCharacterInstance->IsPC() && pCharacterInstance->GetLevel() > 116 &&  pCharacterInstance->GetLevel() <= 118)
	{
			pTextTail->pSaltanatFlagInstance = CGraphicImageInstance::New();
			char szPath[256];
	...
	else if (pCharacterInstance->IsPC() && pCharacterInstance->GetLevel() > 118 &&  pCharacterInstance->GetLevel() <= 119)
	{
			pTextTail->pSaltanatFlagInstance = CGraphicImageInstance::New();
			char szPath[256];
	...
	else if (pCharacterInstance->IsPC() && pCharacterInstance->GetLevel() > 119 &&  pCharacterInstance->GetLevel() <= 120)
	{
			pTextTail->pSaltanatFlagInstance = CGraphicImageInstance::New();
			char szPath[256];
...
void CPythonTextTail::DeleteTextTail(TTextTail * pTextTail)
{
	...
	if (pTextTail->pGameMasterIconInstance)
	{
		CGraphicImageInstance::Delete(pTextTail->pGameMasterIconInstance);
		pTextTail->pGameMasterIconInstance = NULL;
	}
#endif
