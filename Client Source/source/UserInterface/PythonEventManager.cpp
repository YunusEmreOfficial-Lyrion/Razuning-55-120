	...
	switch (iEventType)
	{
		...
		case EVENT_TYPE_QUEST_BUTTON:
		{
			...
			if (!strcmp(name, "조각난 용혼석?....")) {
				PyCallClassMemberFunc(m_poGame, "BINARY_RecvQuest", Py_BuildValue("(isss)", idx, name, "highlight", ""));
			}
			else {
				if (c_rstFile.empty())
				{
					PyCallClassMemberFunc(m_poGame, "RecvQuest", Py_BuildValue("(is)", idx, name));
				}
				else
				{
					PyCallClassMemberFunc(m_poGame, "BINARY_RecvQuest", 
...
void CPythonEventManager::SetGameWindow(PyObject * poGame)
{
	m_poGame = poGame;
}
