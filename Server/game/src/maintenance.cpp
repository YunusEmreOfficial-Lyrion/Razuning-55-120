...
EVENTFUNC(maintenanceDown_event)
{
	...
	else
	{
		...
		std::auto_ptr<SQLMsg> pmsg(DBManager::instance().DirectQuery(sTime));
...
void MaintenanceManager::Send_Text(LPCHARACTER ch, const char* reason)
{
	...
	if (!strcmp(reason, "rmf")) 
	{
		...
		return;
			...
			std::auto_ptr<SQLMsg> reasonReplace(DBManager::instance().DirectQuery(sReason));	
...
void MaintenanceManager::Send_CheckTable(LPCHARACTER ch)
{
	...
	std::auto_ptr<SQLMsg> pMsg(DBManager::instance().DirectQuery("SELECT time,duration FROM player.maintenance LIMIT 1"));	
