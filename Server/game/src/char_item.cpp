...
bool CHARACTER::DoRefineWithScroll(LPITEM item)
{
		...
		pkItemScroll->GetValue(0) == YAGONG_SCROLL) // 현철, 용신의 축복서, 야공의 비전서  처리
	{
		...
		else if (pkItemScroll->GetValue(0) == HYUNIRON_CHN)	// hyuniron_chn = b???metal
		{
			// herhangi bir sans unsuru yok temel success_probdan cekiyor.
		}
		else
		{
...
bool CHARACTER::UseItemEx(LPITEM item, TItemPos DestCell)
{
	...
	switch (item->GetType())
	{
		...
		case ITEM_CAMPFIRE:
			{
				...
				{
					ChatPacket(CHAT_TYPE_INFO, "Bu kadar hizli olamaz... 60 saniye bekle.");
					return false;
				}
		...
		if (!item->IsEquipped())
			...
			{
				ChatPacket(CHAT_TYPE_INFO, "Bunu yapabilmek i?n %d saniye bekleyin.", (GetQuestFlag("ARMOR.CHECKER") - get_global_time()) % 60);
				return false;
			}
			EquipItem(item);
		...
		case ITEM_SKILLBOOK:
			{
				if (IsPolymorphed())
				{
					ChatPacket(CHAT_TYPE_INFO, LC_TEXT("º?½AAß¿¡´A A￥A≫ A?A≫¼o ¾ø½A´I´U."));
					return false;
				}
				...
				else
				{
					...
					{
						// ≫o·I¿i ¼o·A¼­´A value 0 ¿¡ ½ºA³ ¹øE￡°¡ AOA¸¹C·I ±×°IA≫ ≫c¿e.
									...
									else
									{
										...
										if (item->GetVnum() == 71151 || item->GetVnum() == 76023)
										{
												...
												|| (item2->GetType() == ITEM_ARMOR && item2->GetSubType() == ARMOR_BODY))
											{
												...
												if (false == bCanUse)
												{
													ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Hata."));
													break;
												}
											}
											else
...
bool CHARACTER::UseItem(TItemPos Cell, TItemPos DestCell)
{
	...
	if (ITEM_BELT == item->GetType())
		ERROR_MSG(CBeltInventoryHelper::IsExistItemInBeltInventory(this), "?ce kemer slotundaki itemleri almal??.");
...
bool CHARACTER::EquipItem(LPITEM item, int iCandidateCell)
{
	...
	if (iWearCell == WEAR_BODY && IsRiding() && (item->GetVnum() >= 11902 && item->GetVnum() <= 11911 && item->GetVnum() <= 11912)) //Damatl?lar
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("말을 탄 상태에?예복을 입을 수 없습니다."));
		return false;
	}
...
void CHARACTER::BuffOnAttr_ValueChange(BYTE bType, BYTE bOldValue, BYTE bNewValue)
{
	...
	else if(0 == bOldValue)
	{
		...
		if (m_map_buff_on_attrs.end() == it)
		{
			...
			case POINT_ENERGY:
				{
					static BYTE abSlot[] = { WEAR_BODY, WEAR_HEAD, WEAR_COSTUME_MOUNT, WEAR_FOOTS, WEAR_WRIST, WEAR_WEAPON, WEAR_NECK, WEAR_EAR, WEAR_SHIELD };
