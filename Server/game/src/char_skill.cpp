...
struct FuncSplashDamage
{
	...
	void operator () (LPENTITY ent)
	{
		...
		switch (m_pkSk->bSkillAttrType)
		{
			...
			case SKILL_ATTR_TYPE_MAGIC:
				...
				if (m_pkChr->GetJob() == JOB_SURA || m_pkChr->GetJob() == JOB_SHAMAN) {
					iDam = iDam * (85 - pkChrVictim->GetPoint(POINT_RESIST_MAGIC)) / 100;
				}
				else {
		...
		if (pkChrVictim->IsPC())
		{
			if (m_pkSk->dwVnum == SKILL_SANGONG && pkChrVictim->GetLevel() < 125) {
				iDam = (iDam / 100)*(125 - pkChrVictim->GetLevel());
			}
		}
#endif
...
void CHARACTER::ClearSkill() //beceri s??lama fix.
{
