...
ACMD(do_ruhoku)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("￠￥U￠￢¡I ¡ÆA¡¤￠®Aⓒ￢(A¡E¡Æi,¡¾ⓒøE?,¡ioA￠®)￠?￠®￠￥A ¡ÆⓒøAI¡ioA￠®A¡i ¡ic￠?eCO ¨uo ¨uⓒª¨oA￠￥I￠￥U."));
		return;
	}
...
//		case POINT_CURSE_PCT:	return LC_TEXT("");
		...
		case POINT_MALL_ITEMBONUS:		return LC_TEXT("아이템 드롭율 %d배");
		case POINT_MALL_GOLDBONUS:		return LC_TEXT("돈 드롭율 %d배");
...
ACMD(do_ride)
{
	...
	ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("富? ?? 家券秦林技?."));
}
...
ACMD(do_stat_val)
{
	...
	if (ch->IsPolymorphed())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Donu?tu?unuz surece Statu veremessiniz. || Metin2 House"));
		return;
	}
