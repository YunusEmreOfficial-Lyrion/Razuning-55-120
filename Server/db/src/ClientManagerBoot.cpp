...
bool CClientManager::InitializeMobTableTXT()
{
	...
	if (!nameData.Load("mob_names.txt",'\t'))
	{
		fprintf(stderr, "mob_names.txt dosyası yuklenirken bir hata olu?tu\n");
		isNameFile = false;
	} 
	else 
	{
	...
	if (!test_data.Load("mob_proto_test.txt", '\t'))
	{
		fprintf(stderr, "mob_proto_test.txt dosyası yuklenirken bir hata olu?tu. ??lem durduruldu.\n");
		isTestFile = false;
	}
	...
	if (isTestFile) 
	{
		...
		while (test_data.Next()) 
		{
			if (!Set_Proto_Mob_Table(test_mob_table, test_data, localMap))
			{
				fprintf(stderr, "Calı?ma tablosuna giri? eklenemedi.\n");
			}
	...
	if (!data.Load("mob_proto.txt", '\t')) 
	{
		fprintf(stderr, "mob_proto.txt dosyası yuklenirken bir hata olu?tu.\n");
		return false;
	}
	data.Next();
	...
	if (!m_vec_mobTable.empty())
	{
		sys_log(0, "Yeniden Yukleme: mob_proto");
		m_vec_mobTable.clear();
	}
	m_vec_mobTable.resize(data.m_File.GetRowCount() - 1 + addNumber);
	memset(&m_vec_mobTable[0], 0, sizeof(TMobTable) * m_vec_mobTable.size());
...
bool CClientManager::InitializeItemTableTXT()
{
	...
	if(!nameData.Load("item_names.txt",'\t'))
	{
		fprintf(stderr, "item_names.txt ÆAAIA≫ A?¾i¿AAo ¸øCß½A´I´U\n");
		isNameFile = false;
	} 
	else 
	{
	...
	if(!test_data.Load("item_proto_test.txt",'\t'))
	{
		fprintf(stderr, "item_proto_test.txt ÆAAIA≫ A?¾i¿AAo ¸øCß½A´I´U\n");
		//return false;
	} 
	else 
	{
		...
		while(test_data.Next()) 
		{
			if (!Set_Proto_Item_Table(test_item_table, test_data, localMap))
			{
				fprintf(stderr, "¾ÆAIAU CA·IAa A×AIºi ¼AÆA ½CÆ?.\n");
			}
	...
	if(!data.Load("item_proto.txt",'\t'))
	{
		fprintf(stderr, "item_proto.txt ÆAAIA≫ A?¾i¿AAo ¸øCß½A´I´U\n");
		return false;
	}
	data.Next();
